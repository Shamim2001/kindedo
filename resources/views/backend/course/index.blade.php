...
@section('content')
    <div class="">
        <div class="text-end mb-4">
            <a href="{{ route('course.create') }}" class="btn btn-dark">Add New</a>
        </div>
        <!-- Table Dark -->
        <table class="table table-bordered align-middle table-wrap">
            ...
            <tbody>
                ...
                <tr>
                    <td style="width: 5%" scope="row" class="text-center">
                        {{ $courses->perPage() * ($courses->currentPage() - 1) + ++$key }}
                    </td>
                    <td style="width: 10%">
                        <img src="{{ getAssetUrl($course->image, 'uploads/courses') }}" alt="{{ $course->name }}"
                            class="avatar-md">
                    </td>
                    <td style="width: 25%">
                    ...
                    <td style="width: 25%">{{ $course->teacher?->name }}</td>
                    <td style="width: 20%">{{ $course->start_time }}</td>


                    <td style="width: 10%" class="text-center">
                        <div class="hstack gap-3 flex-wrap justify-content-center">

                            <a href="{{ route('course.edit', $course->id) }}" class="btn btn-outline-primary btn-icon btn-sm cursor-pointer"><i
                            ...
                            <form id="delete-form-{{ $course->id }}" action="{{ route('course.destroy', $course) }}"
            ...
            @empty
                <tr>
                    <td colspan="6" class="text-center">No course(s) found!</td>
                </tr>
            @endforelse
