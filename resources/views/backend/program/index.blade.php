...
@section('content')
    <div class="text-end mb-3">
        <a href="{{ route('program.create') }}" class="btn btn-dark">Add New</a>
    </div>
    <!-- Table Dark -->
        ...
        <tbody>
            @forelse ($programs as $key => $program)
                <tr>
                    ...
                    <td style="width: 10%">
                        <img src="{{ getAssetUrl($program->image, 'uploads/programs') }}" alt="" class="avatar-sm">

                    </td>
                    <td style="width: 30%">
                        <a href="{{ route('program.edit', $program->id) }}">{{ $program->name }}</a>
                    </td>
                    ...
                    <td style="width: 10%" class="text-center">
                        <div class="hstack gap-3 flex-wrap justify-content-center">
                            ...
                            <a href="{{ route('program.edit', $program->id) }}"
                                class="btn btn-outline-primary btn-icon btn-sm cursor-pointer"><i
                            ...
                            <a data-bs-toggle="tooltip" data-bs-custom-class="danger-tooltip" data-bs-placement="top"
                                ...
                                onclick="deleteRecord({{ $program->id }})"
                                class="btn btn-outline-danger btn-icon btn-sm"><i class="ri-delete-bin-line"></i></a>


                            <form id="delete-form-{{ $program->id }}" action="{{ route('program.destroy', $program) }}"
