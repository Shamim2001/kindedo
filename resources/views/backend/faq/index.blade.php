...
@section('content')
    <div class="bg-white p-4">
        <div class="text-end mb-4">
            <a href="{{ route('faq.create') }}" class="btn btn-dark">Add New</a>
        </div>
        <!-- Table Dark -->
        <table class="table table-bordered align-middle table-nowrap">
            ...
            @forelse ($faqs as $key => $faq)
                <tr>
                    ...
                    <td style="width: 10%">{{ $faq->title }}</td>
                    <td style="width: 35%">{!! \Str::of($faq->content)->limit(100) !!}</td>
                    <td style="width: 10%" class="text-center">
                        <div class="hstack gap-3 flex-wrap justify-content-center">

                            <a data-bs-toggle="tooltip" data-bs-custom-class="info-tooltip" data-bs-placement="top"
                                data-bs-original-title="Edit" href="{{ route('faq.edit', $faq) }}"
                            ...
                            <a data-bs-toggle="tooltip" data-bs-custom-class="danger-tooltip" data-bs-placement="top"
                                ...
                                onclick="deleteRecord({{ $faq->id }})" class="link-danger fs-base"><i
                                    class="ri-delete-bin-line"></i></a>


                            <form id="delete-form-{{ $faq->id }}"
                                  action="{{ route('faq.destroy', $faq) }}" method="POST"
                                  style="display: none">
                                @csrf
                                @method('DELETE')
                            </form>
